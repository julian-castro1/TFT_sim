...
export async function parseArduinoCode(code) {
    const result = {
        screens: [],
        states: [],
        variables: [],
...
// Extract screens from drawing functions
function extractScreens(lines, functions) {
    const screens = [];
...
// Extract UI elements from function body
function extractUIElementsFromFunction(lines, functionName) {
    const elements = [];
    const touchZones = [];
...
function parseTFTCommand(line, id, lineNumber) {
    const trimmedLine = line.trim();
    // fillScreen
...
function parseTouchZone(line, lines, lineIndex) {
...
function validateParsedDisplay(display) {
